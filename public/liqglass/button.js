class Button extends Container {
  ...
  static measureText(text, fontSize) {
    // If no text, return minimal dimensions
    if (text === '' || text === null || text === undefined) {
      return { width: fontSize * 2 } // Minimal width for empty buttons
    }
  ...
  createTextElement() {
    ...
    this.textElement.className = 'glass-button-text button-text'
    this.textElement.textContent = this.text
    this.textElement.style.fontSize = this.fontSize + 'px'
    
    // Hide text element if no text provided (for children mode)
